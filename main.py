...
def main(genomes, config):
    ...
    for _, g in genomes:
        net = nets.nn.FeedForwardNetwork.create(g, config)
        nets.append(net)
...
def run(configp):
    config = neat.config.Config(neat.DefaultGenome, neat.DefaultReproduction, neat.DefaultSpeciesSet, neat.DefaultStagnation, configp)
...
if __name__ == "__main__":
    ...
    configp = os.path.join(local_dir, 'config.txt')
    run(configp)
